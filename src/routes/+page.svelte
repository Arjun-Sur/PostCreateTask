<script>
    ...
    function updateGradebook(params = {}) {
        ...
        for (let i = 0; i < assignmentsArr.length; i++) { // Iterate through assignments array, adding each assignment to the table header
            ...
            const removeAssignmentButton = createElementWithAttributes("button", { innerHTML: "X", value: i, name: "removeAssignment", className: "delete" });
        ...
        Object.entries(categoryPoints).forEach(([category, points]) => {
            const categoryHeaderCell = createElementWithAttributes("th", { className: "assignment", innerHTML: category });
        ...
        assignmentCategoriesRowArr.forEach(category => { // Iterate through each assignment category, adding each assignment category to the "Category" row
            ...
            const assignmentCategoryCell = createElementWithAttributes("td", { className: "category", innerHTML: category });
        ...
        studentsArr.forEach((student, index) => { // Iterate through each student, calculating and showing their overall, assignment, and category scores while contributing to average data
            ...
            const removeStudentButton = createElementWithAttributes("button", { innerHTML: "X", value: index, name: "removeStudent", className: "delete" });
        ...
        <tr>
            ...
            <td>
                <form id="addAssignment">
                    ...
                    <input type="number" min="1" id="assignmentPoints" required placeholder="Enter points...">
                    <br>
